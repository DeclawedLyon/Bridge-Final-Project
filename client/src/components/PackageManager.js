...
export default function PackageManager() {
  ...
  const newPackage = (event) => {
    ...
    axios
      .post(`/packages/add_item?tracking_number=${state.trkNumNew}&nickname=${state.newNickname}&description=${state.newDescription}`) 
      .then((response) => {
      ...
      >Add a Package +
        ...
        <form id="add-package-form" onSubmit={newPackage}>
          ...
          <input
            ...
            onChange={event => {
              setState(prev => ({...prev, newDescription: event.target.value}))
              }}
          />
        <button 
          type="submit" 
